...
class TextReportGenerator(ReportGenerator):
    ...
    def generate(self, analysis: AnalysisResult, output_path: Optional[Path] = None) -> str:
        ...
        def format_currency_euro(value: float) -> str:
        ...
        report_lines.append(f"Total de Receitas:  {format_currency_euro(analysis.total_income):>12}")
        report_lines.append(f"Total de Despesas:  {format_currency_euro(analysis.total_expenses):>12}")
        report_lines.append(f"Saldo do Período:   {format_currency_euro(analysis.net_flow):>12}")
        report_lines.append("")
        ...
        if analysis.categories_summary:
            ...
            for category, amount in analysis.categories_summary.items():
                ...
                report_lines.append(
                    f"{category_name:<25} {format_currency_euro(amount):>10} ({percentage:>5.1f}%)"
                )

            report_lines.append("-" * 40)
            report_lines.append(f"{'Total Categorizado:':<25} {format_currency_euro(total_categorized):>10}")
            report_lines.append("")
        ...
        if analysis.monthly_summary:
            ...
            for month, data in sorted(analysis.monthly_summary.items()):
                ...
                report_lines.append(
                    f"{month_name:<10} "
                    f"{format_currency_euro(data['income']):>15} "
                    f"{format_currency_euro(data['expenses']):>15} "
                    f"{format_currency_euro(data['balance']):>15}"
                )
